...
function nonsolid_rotation(r_m :: Real, θ :: Real, star :: Star, mag :: Magnetosphere)
	r = r_m*sin(θ)^2
	R = r*sin(θ)
...
function velocity_jacobian(r_m :: Real, θ :: Real, v_t :: Real, v_p :: Real, star :: Star, mag :: Magnetosphere)
	r = r_m*sin(θ)^2
	R = r*sin(θ)
	η = eta(mag.M_dot, mag.r_mi, mag.r_mo, star)
	B_star = surfmagfield(mag.M_dot, mag.r_mi, star)
	Bp = dipolefield(B_star, r, θ)
	∇Bp = dipolefieldgradient(B_star, r, θ)
	v_ff = freefallvelocity(r, θ, star)
	...
	P = Polynomial([ρ^2*E, -2R*ρ*E, (R^2*E - ρ^2*v_eq^2 - (Bp*R/(4*π*η))^2), 
					2*R*ρ*v_eq^2, -R^2*v_eq^2])
	dP = derivative(P)
	∇P = @. Polynomial(vcat(2*ρ*∇ρ + ρ^2*∇E, -2(∇R*E*ρ + R*∇E*ρ + R*E*∇ρ), 
						 (2R*∇R*E + R^2*∇E - 2v_eq^2*ρ*∇ρ - 2Bp*R/(4*π*η)^2*(∇Bp*R + Bp*∇R)),
...
function solvemag(star :: Star, mag :: Magnetosphere; n_r = 5 :: Int, n_θ = 20 :: Int, file = "rotmag.dat")
	...
	for i=1:n_r
		...
		for j=1:n_θ
			...
			v_t = nonsolid_rotation(r_m, θ, star, mag)
			v_t_grid[j,i] = v_t
